...
/*!
 @brief Implements the classic fuzzy search query.The similarity measurement
...
/*!
 ...
 @param minimumSimilarity a value between 0 and 1 to set the required similarity   between the query term and the matching terms. For example, for a
     <code>
  minimumSimilarity </code>  of  <code> 0.5 </code>  a term of the same length   as the query term is considered similar to the query term if the edit distance
    between both terms is less than 
  <code> length(term)*0.5 </code>    <p>
    Alternatively, if 
  <code> minimumSimilarity </code>  is  &gt; = 1f, it is interpreted    as a pure Levenshtein edit distance. For example, a value of  <code> 2f </code>   will match all terms within an edit distance of  <code> 2 </code>  from the    query term. Edit distances specified in this way may not be fractional.
 ...
 @throw IllegalArgumentExceptionif minimumSimilarity is &gt;= 1 or &lt; 0
...
inline jfloat OrgApacheLuceneSandboxQueriesSlowFuzzyQuery_get_defaultMinSimilarity(void);
...
inline jint OrgApacheLuceneSandboxQueriesSlowFuzzyQuery_get_defaultPrefixLength(void);
...
inline jint OrgApacheLuceneSandboxQueriesSlowFuzzyQuery_get_defaultMaxExpansions(void);
