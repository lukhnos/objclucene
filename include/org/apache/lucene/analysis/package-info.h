...
/*!
 @brief Text analysis.
 <p>API and code to convert text into indexable/searchable tokens.  Covers <code>org.apache.lucene.analysis.Analyzer</code> and related classes.</p>
 ...
 &ndash; a deeper <i>Analysis</i> may be needed. Lucene includes both
  ...
  <ul>
    <li>
      <code>org.apache.lucene.analysis.Analyzer</code> &ndash; An <code>Analyzer</code> is 
      responsible for supplying a     
 <code>org.apache.lucene.analysis.TokenStream</code> which can be consumed
      by the indexing and searching processes.  See below for more information
      on implementing your own <code>org.apache.lucene.analysis.Analyzer</code>. Most of the time, you can use
      an anonymous subclass of <code>org.apache.lucene.analysis.Analyzer</code>.
    </li>
    <li>
      <code>org.apache.lucene.analysis.CharFilter</code> &ndash; <code>CharFilter</code> extends
      <code>java.io.Reader</code> to transform the text before it is
      ...
      that are not the same as those in the original text. <code>org.apache.lucene.analysis.Tokenizer.setReader(java.io.Reader)</code>
      accept <code>CharFilter</code>s.  <code>CharFilter</code>s may
      be chained to perform multiple pre-tokenization modifications.   
 </li>
    <li>
      <code>org.apache.lucene.analysis.Tokenizer</code> &ndash; A <code>Tokenizer</code> is a 
      <code>org.apache.lucene.analysis.TokenStream</code> and is responsible for
      breaking up incoming text into tokens. In many cases, an <code>org.apache.lucene.analysis.Analyzer</code> will
      use a <code>org.apache.lucene.analysis.Tokenizer</code> as the first step in the analysis process.  However,
      to modify text prior to tokenization, use a <code>org.apache.lucene.analysis.CharFilter</code> subclass (see
      above).   
 </li>
    <li>
      <code>org.apache.lucene.analysis.TokenFilter</code> &ndash; A <code>TokenFilter</code> is
      a <code>org.apache.lucene.analysis.TokenStream</code> and is responsible
  ...
  <p>
    The relationship between <code>org.apache.lucene.analysis.Analyzer</code> and 
    <code>org.apache.lucene.analysis.CharFilter</code>s,
    <code>org.apache.lucene.analysis.Tokenizer</code>s,
    and <code>org.apache.lucene.analysis.TokenFilter</code>s is sometimes confusing. To ease
    this confusion, here is some clarifications: 
 </p>
  <ul>
    <li>
      The <code>org.apache.lucene.analysis.Analyzer</code> is a
      ...
      to produce <code>org.apache.lucene.analysis.TokenStream</code>s that accept a
      reader and produces tokens, and to wrap or otherwise
      pre-process <code>java.io.Reader</code> objects.
    </li>
    <li>
    The <code>org.apache.lucene.analysis.CharFilter</code> is a subclass of
   <code>java.io.Reader</code> that supports offset tracking.
    </li>
    <li>The<code>org.apache.lucene.analysis.Tokenizer</code>
      is only responsible for <u>breaking</u> the input text into tokens.
    </li>
    <li>The<code>org.apache.lucene.analysis.TokenFilter</code> modifies a
    stream of tokens and their contents.   
 </li>
    <li>
      <code>org.apache.lucene.analysis.Tokenizer</code> is a <code>org.apache.lucene.analysis.TokenStream</code>, 
      but <code>org.apache.lucene.analysis.Analyzer</code> is not.
    </li>
    <li>
      <code>org.apache.lucene.analysis.Analyzer</code> is "field aware", but 
      <code>org.apache.lucene.analysis.Tokenizer</code> is not. <code>org.apache.lucene.analysis.Analyzer</code>s may
      take a field name into account when constructing the <code>org.apache.lucene.analysis.TokenStream</code>.
    </li>
  </ul>
  <p>
    ...
    create an anonymous subclass of <code>org.apache.lucene.analysis.Analyzer</code>, provide <code>org.apache.lucene.analysis.Analyzer.createComponents(String)</code>
  and perhaps also
    <code>org.apache.lucene.analysis.Analyzer.initReader(String,
   java.io.Reader)</code>
 . However, if you need the same set of components
    over and over in many places, you can make a subclass of   
 <code>org.apache.lucene.analysis.Analyzer</code>. In fact, Apache Lucene
 ...
 <a href="{@@docRoot}/../analyzers-common/org/apache/lucene/analysis/custom/CustomAnalyzer.html">CustomAnalyzer</a>
  ...
  <p>
   ...
   <code>org.apache.lucene.document.Field</code>s.  The PerFieldAnalyzerWrapper can be used to associate a different <code>Analyzer</code> with different
   <code>org.apache.lucene.document.Field</code>s. There is a great deal of
  ...
  <PRE class="prettyprint" id="analysis-workflow">
      ...
      try {
        ...
        while (ts.incrementToken()) {
          // Use <code>org.apache.lucene.util.AttributeSource.reflectAsString(boolean)</code>
...
@endcode
 ...
 <ol>
  ...
  <p>
    Creating your own Analyzer is straightforward. Your Analyzer should subclass <code>org.apache.lucene.analysis.Analyzer</code>. It can use
...
@endcode
 ...
 <code>Analyzer.getPositionIncrementGap(fieldName)</code>:
  </p>
  <PRE class="prettyprint">
    Version matchVersion = Version.LUCENE_XY; // Substitute desired Lucene version for XY
...
@endcode
  ...
  <p>
     At the ends of each field, Lucene will call the <code>org.apache.lucene.analysis.TokenStream.end()</code>.
  ...
  <p>
     The base method<code>org.apache.lucene.analysis.TokenStream.end()</code> sets PositionIncrement to 0, which is required.
 ...
 <code>position increment</code> of all tokens to be one.
...
@endcode
 ...
 <code>position length</code> of one.
  ...
  <ul>
    ...
    <li>Tokenizers must call <code>org.apache.lucene.util.AttributeSource.clearAttributes()</code>
  in
    incrementToken().</li>
    <li>Tokenizers must override <code>org.apache.lucene.analysis.TokenStream.end()</code>
 , and pass the final
    offset (the total number of input characters processed) to both
    parameters of <code>org.apache.lucene.analysis.tokenattributes.OffsetAttribute.setOffset(int, int)</code>.</li>
  </ul>
  <p>
  ...
  <ul>
    ...
    <li>When they add tokens, token filters should call <code>org.apache.lucene.util.AttributeSource.clearAttributes()</code> first.</li>
  ...
  <p>
    Classes <code>org.apache.lucene.util.Attribute</code> and 
    <code>org.apache.lucene.util.AttributeSource</code> serve as the basis upon which 
 ...
 <code>org.apache.lucene.analysis.tokenattributes.CharTermAttribute</code> 
    contains the term text of a token, and    
 <code>org.apache.lucene.analysis.tokenattributes.OffsetAttribute</code> contains
  ...
  <table rules="all" frame="box" cellpadding="3" summary="common bundled attributes">
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.CharTermAttribute</code></td>
      <td>
        The term text of a token.  Implements <code>java.lang.CharSequence</code> 
        (providing methods length() and charAt(), and allowing e.g. for direct
        use with regular expression <code>java.util.regex.Matcher</code>s) and 
        <code>java.lang.Appendable</code> (allowing the term text to be appended to.)
      </td>
    </tr>
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.OffsetAttribute</code></td>
      <td>The start and end offset of a token in characters.</td>
    </tr>
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.PositionIncrementAttribute</code></td>
      <td>See above for detailed information about position increment.</td>
    </tr>
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.PositionLengthAttribute</code></td>
      <td>The number of positions occupied by a token.</td>
    </tr>
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.PayloadAttribute</code></td>
      <td>The payload that a Token can optionally have.</td>
    </tr>
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.TypeAttribute</code></td>
      <td>The type of the token. Default is 'word'.</td>
    </tr>
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.FlagsAttribute</code></td>
      <td>Optional flags a token can have.</td>
    </tr>
    <tr>
      <td><code>org.apache.lucene.analysis.tokenattributes.KeywordAttribute</code></td>
      <td>
 ...
 <h4 id="analysis-lifetime">Token Stream Lifetime</h4>
  ...
  opposite. <code>org.apache.lucene.analysis.Analyzer</code> applies a reuse
  strategy to the tokenizer and the token filters. It will reuse
  them. For each new input, it calls <code>org.apache.lucene.analysis.Tokenizer.setReader(java.io.Reader)</code> 
  to set the input. Your components must be prepared for this scenario,
  as described below. 
 <h4>Tokenizer</h4>
  <ul>
    <li>
    You should create your tokenizer class by extending <code>org.apache.lucene.analysis.Tokenizer</code>.
    </li>
    <li>
    Your tokenizer <strong>must</strong> override <code>org.apache.lucene.analysis.TokenStream.end()</code>.
    ...
    <li>
    If your tokenizer overrides <code>org.apache.lucene.analysis.TokenStream.reset()</code>
    or <code>org.apache.lucene.analysis.TokenStream.close()</code>, it
    <strong>must</strong> call the corresponding superclass method.
    </li>
  </ul>
  <h4>Token Filter</h4>
    You should create your token filter class by extending <code>org.apache.lucene.analysis.TokenFilter</code>.
    If your token filter overrides <code>org.apache.lucene.analysis.TokenStream.reset()</code>,
    <code>org.apache.lucene.analysis.TokenStream.end()</code>
    or <code>org.apache.lucene.analysis.TokenStream.close()</code>, it
    <strong>must</strong> call the corresponding superclass method. 
 <h4>Creating delegates</h4>
    Forwarding classes (those which extend <code>org.apache.lucene.analysis.Tokenizer</code> but delegate
    selected logic to another tokenizer) must also set the reader to the delegate in the overridden   
 <code>org.apache.lucene.analysis.Tokenizer.reset()</code> method, e.g.:
    <pre class="prettyprint">
...
@endcode
 ...
 &#47;
 **    * Create a new LengthFilter. This will filter out tokens whose    * CharTermAttribute is either too short    * (&lt; min) or too long (&gt; max).    * &commat;&commat;param version the Lucene match version    * &commat;&commat;param in      the TokenStream to consume    * &commat;&commat;param min     the minimum length    * &commat;&commat;param max     the maximum length    *&#47;  public LengthFilter(Version version, TokenStream in, int min, int max) {     super(version, in);     this.min = min;     this.max = max;   }    &commat;&commat;Override   public boolean accept() {     final int len = termAtt.length();     return (len &gt;= min &amp;&amp; len &lt;= max);   }  } 
@endcode <p>   In LengthFilter, the CharTermAttribute is added and stored in the instance   variable <code>termAtt</code>.  Remember that there can only be a single   instance of CharTermAttribute in the chain, so in our example the   <code>addAttribute()</code> call in LengthFilter returns the   CharTermAttribute that the WhitespaceTokenizer already added. </p> <p>   The tokens are retrieved from the input stream in FilteringTokenFilter's    <code>incrementToken()</code> method (see below), which calls LengthFilter's   <code>accept()</code> method. By looking at the term text in the   CharTermAttribute, the length of the term can be determined and tokens that   are either too short or too long are skipped.  Note how   <code>accept()</code> can efficiently access the instance variable; no    attribute lookup is necessary. The same is true for the consumer, which can   simply use local references to the Attributes. </p> <p>   LengthFilter extends FilteringTokenFilter: </p>  <pre class="prettyprint"> public abstract class FilteringTokenFilter extends TokenFilter {    private final PositionIncrementAttribute posIncrAtt = addAttribute(PositionIncrementAttribute.class);    &#47;
 **    * Create a new FilteringTokenFilter.    * &commat;&commat;param in      the TokenStream to consume    *&#47;   public FilteringTokenFilter(Version version, TokenStream in) {     super(in);   }    &#47;** Override this method and return if the current input token should be returned by incrementToken. *&#47;
...
@endcode
 ...
 <code>org.apache.lucene.util.AttributeFactory</code>. The factory accepts an Attribute interface as argument
 ...
 <code>org.apache.lucene.util.AttributeImpl</code>:
  </p>
  <pre class="prettyprint">
...
@endcode
  ...
  Analyzers take Java <code>java.io.Reader</code>s as input. Of course you can wrap your Readers with <code>java.io.FilterReader</code>s
  ...
  text. 
 <p>
  <code>org.apache.lucene.analysis.CharFilter</code> is designed to allow you to pre-process input like a FilterReader would, but also
...
#pragma push_macro("INCLUDE_ALL_OrgApacheLuceneAnalysisPackage_info")
#ifdef RESTRICT_OrgApacheLuceneAnalysisPackage_info
#define INCLUDE_ALL_OrgApacheLuceneAnalysisPackage_info 0
#else
#define INCLUDE_ALL_OrgApacheLuceneAnalysisPackage_info 1
#endif
#undef RESTRICT_OrgApacheLuceneAnalysisPackage_info
...
#endif
#pragma pop_macro("INCLUDE_ALL_OrgApacheLuceneAnalysisPackage_info")
